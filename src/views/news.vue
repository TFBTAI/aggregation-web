...
<script>
  export default {
    ...
    methods:{
      async getNodeList() {
         let resultList = await this.$axios.get(`http://localhost:8082/initPage/?category=news`)
            .then(
      ...
      async getNodesSize() {
        let nodeSize = await this.$axios.get(`http://localhost:8082/getNodesSize/?category=news`)
            .then(
                async function(response){
                  return await response.data;
                }
      ...
      async getMoreNodes() {
        if(this.nodeList.length < this.nodeSize){
          let resultList = await this.$axios.get(`http://localhost:8082/getMoreNodes/?category=news`)
              .then(
                  async function(response){
                    return await response.data;
                  }
